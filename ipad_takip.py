...
def mediamarkt_fiyat_kontrol():
    ...
    with sync_playwright() as p:
        ...
        cards = page.query_selector_all('[data-test="mms-search-srp-productlist-item"]')
        print(f"✅ {len(cards)} ürün kartı bulundu")
      
        for card in cards:
            try:
                title_el = card.query_selector('h2')
                price_el = card.query_selector('[data-test="mms-product-price-box"]')
                ...
                if 'iPad Air' in title and 'M4' in title and 'Wi-Fi' in title:
                    ...
                    boyut = '11' if '11' in title else '13' if '13' in title else '?'
                    boyut = boyut + ' inç'
                    urunler.append({'baslik': title, 'fiyat': price, 'boyut': boyut, 'kapasite': kapasite})
...
def mail_gonder(urunler):
    ...
    def bul(boyut, kap):
        return next((u for u in urunler if u['boyut'] == boyut and u['kapasite'] == kap), None)
...
if __name__ == "__main__":
    ...
    # Ürün bulunsa da bulunmasa da mail gönder (durumu bilmek için)
    mail_gonder(urunler)
    print("✅ Tamamlandı")
